...
class EnvironmentCanada(NINJSFeedParser):
    """
    Feed Parser for Environment Canada weather reports (EN and FR)
    """

    # Initialize class variables
    label = 'Environment Canada Weather Parser'
    NAME = label.lower()
    ...
    def parse(self, file_path):
        return self.strategy.parse(file_path)
    ...
    def parse(self, file_path, provider=None):
        ...
        with open(file_path, "r", encoding='utf-8') as file:
            ...
            date_match = re.search(r'\d{4} UTC \w{3} \w{3} \d{2} \d{4}', article)
            if date_match:
                date_str = date_match.group(0)
                date_first_created = datetime.datetime.strptime(date_str, '%H%M UTC %a %b %d %Y')
            else:
                self.logger.error("Date not found in the article")
                date_first_created = None

            if 'WEATHER TABLE' in article:
                body = self.format_tabular_data(article)
            elif 'PREVISIONS' in article or 'PREVISIONS MISES A JOUR' in article:
                body = self.format_non_tabular_data(article)
            ...
            item["version"] = "1"
            item["type"] = "text"
            item["byline"] = "The Canadian Press"
            item["versioncreated"] = date_first_created.strftime("%Y-%m-%dT%H:%M:%S+0000") if date_first_created else None
            item["firstcreated"] = date_first_created.strftime("%Y-%m-%dT%H:%M:%S+0000") if date_first_created else None
            item["urgency"] = None
            item["priority"] = None
    ...
    @classmethod
    def format_non_tabular_data(cls, content):
